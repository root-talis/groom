...
At runtime, the [hello-world](examples/hello-world/src/main.rs) example serves the merged YAML at `/spec.yaml` via an axum `Extension`. The [todo example](examples/todo/backend/src/controller/mod.rs) gates this behind a `--serve-spec` flag.
...
The [todo justfile](examples/todo/justfile) defines `generate-api-spec` and chains it before `generate-api-client` (`npx openapi-ts` against `spec.yaml`).
